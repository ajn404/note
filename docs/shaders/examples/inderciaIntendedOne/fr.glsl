...
vec4 map(vec3 p,Grid grid){
    p-=grid.c;
    p.z+=.4*sin(2.*u_time+1.*fract(grid.h.z*28.)+.3*(grid.c.x+grid.c.y));
    ...
    if(grid.i==0){//2x2
        if(grid.h.y<.3){//speaker
            ...
            pt.y-=.5-smoothstep(-.5,.5,sin(u_time+c.x+grid.h.z*100.));
            ...
            float ani=smoothstep(-.5,.5,sin(u_time+grid.h.z*100.));
            ...
            if(d3<d){
                float led=1.-ani;
                led*=.5+.5*sin(u_time*exp2(3.+3.*grid.h.z));
        ...
        if(grid.h.y<.4){//fader
            ...
            float ani=smoothstep(-.2,.2,sin(u_time+grid.h.z*100.));
            pt.x-=mix(-.8,.8,ani);
            ...
            float ani=smoothstep(.8,.9,sin(10.*u_time-c.x*2.2+grid.h.z*100.));
        ...
        if(grid.h.y<.5){//knob
            ...
            float ani=smoothstep(-.5,.5,sin(2.*u_time+grid.h.z*100.));
            pt.xy*=r2d(PI/6.*5.*mix(-1.,1.,ani));
            ...
            float ani=sin(2.*u_time+grid.h.z*100.);
...
vec3 nmap(vec3 p,Grid grid,float dd){
    ...
    void mainImage(out vec4 fragColor,in vec2 fragCoord)
    {
        ...
        float canim=smoothstep(-.2,.2,sin(u_time));
        ...
        if(march.isect.x<1E-2){
            ...
            if(mtl==0.){
                ...
                float lv=texture2D(iChannel0,vec2(march.grid.h.z,0)).x*1.;
                ...
                led*=vec3(2,1,2);
                float plasma=sin(length(c)*10.-10.*u_time+march.grid.h.z*.7);
                plasma+=sin(c.y*10.-7.*u_time);
                led*=.5+.5*sin(plasma);
                col+=2.*led;
                basecol=.04*led;
            }else if(mtl==5.){//808
